...
public class EventLoader {
    ...
    public static void loadEvents(BunnyHub client, JDABuilder jdaBuilder, String packageName) {
        ...
        for (Class<? extends BunnyEvent> clazz : eventClasses) {
            // Anonymous and abstract subclasses are structure, not listeners; the scan
            // returns them and instantiation would fail noisily for no reason. Mirrors
            // the same guard in CommandLoader.
            if (clazz.isAnonymousClass() || java.lang.reflect.Modifier.isAbstract(clazz.getModifiers()))
                continue;
