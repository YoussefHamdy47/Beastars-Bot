...
public class MessageListener extends BunnyEvent {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (command.hasBranches()) {
            ...
            String branch = first[0].toLowerCase();
            ...
            if (group != null) {
                ...
                String action = second[0].toLowerCase();
        ...
        MessageEmbed denial = CommandGate.check(ctx, registry, command, target);
        if (denial != null) {
            ctx.reply(denial);
            return;
        }
